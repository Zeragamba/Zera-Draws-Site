...
class AuthApi extends ServerApi {
  public passkeys = {
    login: async (): Promise<UserData> => {
      ...
      const options = webauthn.parseRequestOptionsFromJSON({
      ...
      const { user, auth_token } = await this.post("/login/passkey", {
        ...
        parseResData: (data) => {
          return z
            .object({ user: UserDataSchema, auth_token: z.string() })
            .parse(data)
        },
      })
    ...
    create: async (): Promise<Omit<RegisterPassKeyParams, "passkey">> => {
      ...
      const options = webauthn.parseCreationOptionsFromJSON({
        publicKey: challengeData,
      })
      return {
        challenge: challengeData.challenge,
    ...
    register: async ({
      ...
      challenge,
      publicKeyCredential,
    }: RegisterPassKeyParams): Promise<PasskeyData> => {
      return this.post("/user/me/passkey", {
        data: { passkey, challenge, publicKeyCredential },
        parseResData: (data) =>
          PasskeyResSchema.transform((data) => data.passkey).parse(data),
      })
    },
    ...
    list: () => {
      return this.get("/user/me/passkey", {
        parseResData: (data) =>
          PasskeyListResSchema.transform((data) => data.passkeys).parse(data),
      })
    },
