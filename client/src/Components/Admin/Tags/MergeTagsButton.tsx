...
import { useMergeTags$ } from '../../Tags/TagsApi/MergeTags'
...
export const MergeTagButton: FC<MergeTagButtonProps> = ({
  ...
  const onMerge = async () => {
    if (!destTagId) return
    const mergedTag = await mergeTags$.mutateAsync({ srcTagId: srcTag.id, destTagId: destTagId })
    setPromptOpen(false)
    onMerged(mergedTag)
  }
        ...
        <DialogContent>
          ...
          {allTags && (
            <FormControl sx={{ minWidth: 200 }}>
              ...
              >
                <MenuItem value="" />
                {allTags.filter(tag => tag.id !== srcTag.id).map(tag => (
        ...
        <DialogActions>
          {mergeTags$.isLoading ? (
            ...
            <>
              ...
              <Button onClick={onMerge} color="error" variant="contained" disabled={!destTagId}>Merge</Button>
            </>
          )}
        </DialogActions>
      </Dialog>
